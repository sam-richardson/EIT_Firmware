...
//  
  ...
  while (bStopFlag == true) // running
  {
    ...
    else if (RxBuffer[0] == 'e'  && RxBuffer[1] == '\n' && mode !=5)  // 16 electrode Tetrapolar Imaging neighbour mode
    {
      mode = 5;
      // Reset everything.  
      adi_GPIO_UnInit();  
      adi_AFE_UnInit(hDevice);
      PRINT("mode 5: 16 electrode imaging NEIGHBOUR\n");
      init_mode_tetramux();
      adi_UART_BufFlush(hUartDevice);
    }    
...
void multiplex_adg732_neighbour(ADI_AFE_DEV_HANDLE  hDevice, const uint32_t *const seq,uint32_t n_el) {
   ...
   //   32, 192, 896  
    ...
    for (uint32_t econf = 0;econf<numberofmeasures;econf++) {    
      ...
      // This is where we select the electrode sequence. i.e. 8,16 or 32 adjacent or opposition.  
      int16_t* e =(int16_t *)electrode_configuration_16_nel16_dist5[econf];
